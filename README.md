...
Initialize the volume container using the `kintesh/openvpn` image with the
...
To generate a client certificate, `kintesh/openvpn` uses EasyRSA via the
`easyrsa` command in the container's path.  The `EASYRSA_*` environmental
variables place the PKI CA under `/etc/openvpn/pki`.

Conveniently, `kintesh/openvpn` comes with a script called `ovpn_getclient`,
...
run time image (kintesh/openvpn) and the `$OVPN_DATA` data volume. To remove
